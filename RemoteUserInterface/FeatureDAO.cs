...
namespace BuildingDataBase
	...
	class FeatureDAO
...

		{
			...
			{
			cmd = connect.CreateCommand();
			cmd.CommandText = "SELECT ROWID,coord FROM Features WHERE edge_id=" + tableid + " AND type=" + ((int) RoomData.FeatureType.OPENING_EDGE);
			reader = cmd.ExecuteReader();
			col = new DataColumn();
			col.ColumnName = "rowid";
			col.DataType = System.Type.GetType("System.Int64");
...

		{
			...
			{
			cmd = connect.CreateCommand();
			cmd.CommandText = "SELECT ROWID,coord FROM Features WHERE recharge_id=" + tableid + " AND type=" + ((int)RoomData.FeatureType.TARGET);
			reader = cmd.ExecuteReader();
			col = new DataColumn();
			col.ColumnName = "rowid";
			col.DataType = System.Type.GetType("System.Int64");
