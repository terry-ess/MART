...
namespace Turn_Calibration
	{
	public partial class TurnCalForm : Form
		{
		...
		private static SemaphoreSlim tool_access = new SemaphoreSlim(1);
			...
			if (tcntl.Connected())
				...
				if (rsp.StartsWith(UiConstants.OK))
					...
					if (tconnect.Connected() && kaconnect.Connected())
						...
						tcntl = null;
						tconnect = null;
						Log.LogEntry("Could not open a required connection.");
						this.Enabled = false;
						}
					}
				else
		...
		private void TurnCalForm_FormClosing(object sender, FormClosingEventArgs e)
