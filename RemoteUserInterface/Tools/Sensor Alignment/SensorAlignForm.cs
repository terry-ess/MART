...
namespace Sensor_Alignment
	{
	public partial class SensorAlignForm : Form
		{
...

		{
			...
			if (rsp.StartsWith(UiConstants.OK))
				{
				if (current_tab == 1)
					HeadTiltAlignCntrl.Close();
				else if (current_tab == 2)
					SensorAlignCntrl.Close();
				if (tabControl1.SelectedIndex == 1)
					HeadTiltAlignCntrl.Open();
				if (tabControl1.SelectedIndex == 2)
					SensorAlignCntrl.Open();
		...
		private void SensorAlignForm_FormClosing(object sender, FormClosingEventArgs e)
