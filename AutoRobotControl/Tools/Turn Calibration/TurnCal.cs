...
namespace Turn_Calibration
	...
	class TurnCal : ToolsInterface
...

		{
			...
			Log.LogEntry("Turn calibration keep alive server started.");
			while (run)
				{
				...
				if (run && (msg.Length > 0))
					...
					if (run)
						...
						if (no_msg_count < UiConstants.INTF_TIMEOUT_COUNT)
							...
							{
							Log.LogEntry("Turn calibration timed out.");
							run = false;
							if ((srvr != null) && srvr.IsAlive)
								srvr.Join();
							kasrvr = null;
			...
			Log.LogEntry("Turn calibration keep alive server closed");
		}
...

		{
			...
			Log.LogEntry("Turn calibration server started.");
			while (run)
				{
						...
						if (values.Length == 2)
							...
							if (new_tab != current_tab)
								...
								switch(new_tab)
									...
									case 1:
										if (BaseParam.Open(connect))
											{
											tab_opened = true;
											MsgHandle = BaseParam.MessageHandler;
											}
										break;
				...
				{
				Log.LogEntry("Turn calibration server exception: " + ex.Message);
				Log.LogEntry("Stack trace: " + ex.StackTrace);
				}

				}
			Log.LogEntry("Turn calibration server stopped.");
		}
